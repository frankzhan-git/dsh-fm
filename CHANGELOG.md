...
- **质量**：新增 GitHub Actions verify 工作流（ubuntu + Node 20，`npm ci` + `npm test`）+ README 徽章；`test` 脚本改为 `node --test` 默认发现（跨平台/跨 Node 版本 glob 兼容）；语法高亮正则由「重复命名分组」改为编号分组 + 平行类名数组（Node < 22 / 部分旧引擎会抛 SyntaxError，Node 20 验证 137 例全通过）
