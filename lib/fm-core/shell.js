...
// 沙箱策略：执行时由调用方（git 域/安装域）经 policy.js 解析并传入 policy；
// 未传入时回退旧行为（danger-full-access），保证无策略环境（测试/旧调用）不中断。
import { norm } from './util.js'
import { FM_LIMITS } from '../../src/shared/contract/fm-limits.js'
...
export function createShell(services) {
  ...
  let gitProbeAt = 0
  const GIT_PROBE_TTL = FM_LIMITS.PROBE_RETRY_MS // 未找到时 30s 内不重复探测

  async function sh(root, command, stdoutMaxBytes, timeoutMs, env, policy) {
    if (shell === undefined) throw new Error('shell 服务不可用')
    const spec = shell.resolve({
      command,
      workdir: root,
      timeoutMs: timeoutMs || FM_LIMITS.HOST_CMD_TIMEOUT_MS,
      stdoutMaxBytes: stdoutMaxBytes || FM_LIMITS.HOST_STDOUT_MAX_BYTES,
      env,
      // 官方政策：由调用方解析（policy.js）；无则回退旧值
      sandboxPolicy: policy || { mode: 'danger-full-access', workspaceRoot: root },
    })
    return shell.run(spec)
  }

  const gitVersionOf = async (bin, root, policy) => {
    const cmd = bin ? quote(bin) + ' --version' : 'git --version'
    const r = await sh(root, cmd, 4096, FM_LIMITS.HOST_PROBE_TIMEOUT_MS, undefined, policy)
    if (r.exitCode !== 0) return null
    return String(r.stdout && r.stdout.text || '').trim() || 'git'
  }

  // 探测 git：PATH → 常见安装路径；结果缓存（TTL 仅作用于「未找到」）
  const probeGit = async (root, policy) => {
    ...
    const v = await gitVersionOf('', root, policy)
    ...
    for (const c of cand) {
      try {
        ...
        if (info && info.type === 'file') {
          gitBin = c
          gitVersionCache = await gitVersionOf(c, root, policy)
  ...
  const gitCmd = async (root, argsStr, o) => {
    const git = await probeGit(root, o && o.policy)
    ...
    return sh(root, cmd, (o && o.stdoutMaxBytes) || FM_LIMITS.HOST_STDOUT_MAX_BYTES, (o && o.timeoutMs) || FM_LIMITS.HOST_CMD_TIMEOUT_MS, env, o && o.policy)
  }
