...
export function createGitIndexHandlers(services) {
  ...
  return {
    'fm-git-index-set': async (args) => {
      ...
      if (checked) {
        // 加入索引（两动作解耦，任一发生即算变更）：
        //   A) 从 .gitignore 移除自身/锚定子项条目（有则写回；git 无法在已忽略目录内反忽略 → 先行报错）
        //   B) 确保进入索引：未跟踪（新文件/整目录）或曾被 rm --cached → git add（已跟踪 no-op）
        // 旧实现用 changed 短路 —— 无忽略条目的普通未跟踪文件会被提前返回，勾选无效（本次修复）
        ...
        // A) 移除 .gitignore 条目
        if (changed) await writeIgnoreText(repoRoot, keep.join('\n').replace(/\n+$/, '\n'))
        // B) 纳入索引：未跟踪 → git add（无论是否移除了条目）
        let added = false
        if (!(await lsTracked())) {
        ...
        if (!changed && !added) return { ok: true, changed: false, rel, checked, recursive }
        if (statusCache) statusCache.invalidate(repoRoot) // 索引/.gitignore 变更 → 失效状态缓存
