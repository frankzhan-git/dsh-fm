...
// 依赖注入式工厂 createFsHandlers({ fs, rootOf, sh, quote, resolvePolicy }) → 文件类 RPC handlers。
// 文件删除走 shell（平台命令）：经官方政策解析（写操作 read-only 会话拒绝），不再绕过策略。
import { norm } from './util.js'
import { tail } from './shell.js'
import { sniffText, TEXT_SNIFF_SAMPLE } from './text-sniff.js'
...
export function createFsHandlers(services) {
  const { fs, rootOf, sh, quote, resolvePolicy } = services

  // 文本读取载荷（已知文本与嗅探兜底共用）：≤ TEXT_LIMIT 全文；更大流式截断（不拒绝）
  const readTextPayload = async (target, info, detected) => {
    let content = ''
    let truncated = false
    ...
    return ok({ kind: 'text', content, truncated, limit: TEXT_LIMIT, size: info.size == null ? content.length : info.size, detected: !!detected })
  }
  ...
  return {
    ...
    'fm-read': async (args) => {
      ...
      if (TEXT_EXT.has(ext) || ext === '') {
        return await readTextPayload(target, info)
      }
      // 兜底：未知扩展名（含 .git-credentials 等点开头文件名）→ 8KB 采样内容嗅探
      // 文本 → 默认文本预览（detected 标记）；二进制（NUL/控制字符强信号）→ 仍不支持
      try {
        const sample = await fs.readBytes(target, undefined, TEXT_SNIFF_SAMPLE)
        if (sniffText(sample)) return await readTextPayload(target, info, true)
      } catch (e) { /* 采样不可用 → 走不支持 */ }
    ...
    'fm-remove': async (args) => {
      ...
      const info = await fs.stat(target)
      if (!info) return { ok: false, error: '文件不存在' }
      // 删除属写操作：read-only 会话拒绝（官方策略模型）
      const pol = resolvePolicy(args && args.sessionId, true)
      if (pol.denied) return failWith(FM_ERROR_CODES.SANDBOX_DENIED, '沙箱策略为只读，无法删除（当前模式 ' + pol.denied + '）')
      ...
      const res = await sh(root, delCmd, 4096, undefined, undefined, pol.policy)
