...
export function createGitInstallHandlers(services) {
  const { fs, rootOf, probeGit, setGitBin, getGitVersion, findGitBin, gitCmd, sh } = services
  ...
  return {
    'fm-git-install-init': async (args) => {
      ...
      let installed = false
      if (git.bin === null) {
        const res = process.platform === 'win32' ? await installGitWindows(root) : await installGitPosix(root)
        if (!res || res.bin === null) {
          return { ok: false, error: 'git 安装失败：' + ((res && res.hint) || '请检查网络后重试，或手动安装 git') }
        }
        await setGitBin(res.bin, root)
        installed = true
      }
