...
import { gitInitWithDefaults } from './git.js'
...
export function createGitInstallHandlers(services) {
  const { fs, rootOf, probeGit, setGitBin, getGitVersion, findGitBin, gitCmd, sh, resolvePolicy, statusCache } = services
  ...
  return {
    'fm-git-install-init': async (args) => {
      ...
      // 初始化改变仓库状态（.git / .gitignore）：失效状态缓存，避免胶囊显示初始化前的陈旧数据
      if (statusCache) statusCache.invalidate(anchor)
