...
// 文件/git 业务逻辑见 ./fm-core/（领域拆分），RPC 契约见 ../src/shared/fm-contract.js。
// 必须声明 inject：在 Loader 架构下，插件的 apply 会在依赖服务提供前先行执行，
// 没有 inject 时 ctx.get('fs') 等全部为 undefined，导致 apply 提前返回、路由从未注册。
import { createFmCore, fail, assertContractCoverage } from './fm-core/index.js'
import { FM_ROUTE } from '../src/shared/fm-contract.js'
...
export default {
  ...
  apply(ctx) {
    ...
    const HANDLERS = assertContractCoverage(createFmCore({ fs, shell, sessions, sp }))
